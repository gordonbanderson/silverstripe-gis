...
class Tile
{
    ...
    public function __construct($z, $x, $y, $defaultStyle = [], $wrap = null, $size = null)
    {
        ...
        $this->wrap = $wrap !== null
            ? $wrap
            : self::config()->get('wrap_date');
        $this->size = $size
            ? $size
            : WebMapTileService::config()->get('tile_size');
    ...
    public function debug($text = null): void
    {
        $this->resource->debug($text ? $text : "$this->z, $this->x, $this->y");
    }
    ...
    public function getRelativePixelCoordinates($wkt, &$reflection = null)
    {
        ...
        // determin rendering offset
        {
            ...
            GIS::each(
                $geo,
                static function ($lonlat) use (&$min, &$max): void {
                    $min = \is_null($min)
                        ? $lonlat[0]
                        : \min($min, $lonlat[0]);
                    $max = \is_null($max)
