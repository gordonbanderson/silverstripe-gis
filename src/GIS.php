...
class GIS
{
    ...
    /** @var \proj4php\Proj4php instance */
    ...
    {
        return self::each($coordinates, static function ($coordinate) use ($fromProj, $toProj) {
    ...
    public function __get($property)
    {
        ...
        switch ($property) {
            ...
            case 'srid':
                return \preg_match('/^SRID=(\d+);/i', $this->value, $matches)
                    ? (int)$matches[1]
                    : null;
            case 'type':
                return \preg_match('/^SRID=\d+;(' . \implode('|', \array_change_key_case(self::TYPES, \CASE_UPPER)) . ')/i', $this->value, $matches)
                    ? self::TYPES[\strtolower($matches[1])]
                    : null;
            case 'coordinates':
            ...
            case 'geometries':
                ...
                if (\preg_match(self::EWKT_PATTERN, $this->value, $matches)) {
                    ...
                    return \array_map(static function ($geometry) use ($srid) {
    ...
    public function __toString()
    {
        ...
        $type = isset($this->value['type'])
            ? \strtoupper($this->value['type'])
            : null;
        $srid = isset($this->value['srid'])
            ? $this->value['srid']
            : GIS::config()->default_srid;
        $array = isset($this->value['coordinates'])
